...
//排序计算kendall tau值
	public static BigDecimal calculateTau(int[] standard, int[] comparison){
		 ...
		 if (standard.length != comparison.length) {
	       ...
	       //BigDecimal kendellTau = BigDecimal.valueOf(1.0).subtract(distance.multiply(BigDecimal.valueOf(4).divide(nBigDecimal, 5, 4)));
	...
	public static void main(String[] args) throws IOException{
	  ...
	  Scanner sc = new Scanner(System.in);
		...
		sc.close();
		//分解好的矩阵
		String rescalEmbending = "D:\\RESCAL\\Ext-RESCAL-master\\Ext-RESCAL-master\\"+ fileName+ "-latent" +latentNumber + "-lambda" + lambdaNumber+".embeddings.txt";
		//BigDecimal[][] matrix1 = RescalDistance.calcEuclideanDistance(rescalEmbending, dimension);
		BigDecimal[][] matrix1 = RescalDistance.calcVectorDistance(rescalEmbending, method);
		System.out.println(matrix1.length);
		ArrayList<Entry<Integer, BigDecimal>>  rescalList = RescalDistance.sortVectorDistance(matrix1, "BigDecimal");
	  ...
	  //StandardMethod.printMatrix(shortF);
		ArrayList<Entry<Integer,Integer>>  standaedList = StandardDistance.sortStandard(shortF);
		String resultFile = "D:\\RESCAL\\Ext-RESCAL-master\\Ext-RESCAL-master\\tauResult\\tauResult-" + fileName +"-latent"+ latentNumber + "-lambda" + lambdaNumber + "-"+ method + ".txt";
	    FileWriter fileWriter = new FileWriter(resultFile);
	    int length = rescalList.size();
		int [] standard = new int[length];
		int [] comparison = new int[length];
		for(int i = 0; i < length; i++){
			standard[i] = standaedList.get(i).getKey();
			comparison[i] = rescalList.get(i).getKey();
			fileWriter.write(standard[i] + "   " + standaedList.get(i).getValue() +" ; " + comparison[i] + "  " + rescalList.get(i).getValue() + "\n");
		}
		fileWriter.close();
	...
	//	System.out.println(calculateTau(standard1, comparison1));
		System.out.println("********************************");
