...
public class ClusterImpl {
	...
	public static HashMap<Integer, int[]> labelPropagation(int[] centroidList, HashMap<Integer, HashSet<Integer>> neighborsMap) {
			...
			while (i.hasNext()) {
				...
				if (!labelMap.containsKey(tempNeighbor)) {
					...
					if (labelMap.get(current)[0] == labelMap.get(tempNeighbor)[0])
						...
						int result = confilctResult[tempNeighbor][labelMap.get(tempNeighbor)[1]]
								.compareTo(confilctResult[tempNeighbor][current]);
						if (result == 0) {
							continue;
						} else if (result == -1) {
							// 当前传播过来的点和A的相似度大
