...
public class ClusterImplementation {
	...
	public static  HashMap<Integer, int[]> getIsolatedNodeCluster(HashSet<Integer> sliceTotalNodes,
		    ...
		    for(Entry<Integer, int[]> isolatedNode : isolatedNodesLabelSet.entrySet()){
		    	...
		    	for(int i = 0; i < centroidList.length; i++){
		    		BigDecimal distance = RescalDistance.calcVectorDistance(entityVectors,
		    				"Cosine-2", isolatedNode.getKey(), centroidList[i]);
		    		if(distance.compareTo(maxDistance) > 0)
		    			maxDistance = distance;
		    			clusterBelongedTo = centroidList[i];
		    	        	
		    	}
		    //	System.out.println("isolated node id-- " + isolatedNode.getKey() + " cluster label-- " + centroidList[i] + " distance " + distance);
	 ...
	 * @return
	 */
	public static HashMap<Integer, HashSet<Integer>> allocateNodestoCluster(HashMap<Integer, int[]> labelMap, HashMap<Integer, HashSet<Integer>> outgoingNeighborsMap, HashMap<Integer, HashSet<Integer>> incomingNeighborsMap ) {
	...
	public static LinkedHashMap<Integer, HashMap<Integer, HashSet<Integer>>> getSliceClusterMap(
		...
		for (Entry<Integer, HashSet<Integer>> slice : sliceNodes.entrySet()) {
			...
			if (slice.getValue().size() != 0) {
				...
				System.out.println("Slice id ******* " + slice.getKey() + " slice size " + slice.getValue().size() +  " ");
	...
	public static void main(String[] args) throws IOException, ParseException{
		...
		HashMap<Integer, HashSet<Integer>> resourceIncomingNeighborMap = GraphUtil.getNodeIncomingNeighbors(rescalInputDir);
		HashMap<Integer, HashSet<Integer>> resourceOutgoingNeighborMap = GraphUtil.getNodeOutgoingNeighbors(rescalInputDir);
